...
*   🧠 **咨询师模式**：当 AI 卡住时，自动生成结构化求助邮件，引入外部干净环境（Gemini/Claude）破局。
...
**最佳实践**：将生成的邮件内容直接发送给 **Gemini 3 Pro**、**Claude 4.5**、**DeepSeek V3.2**、**Qwen Max**。获得基于纯粹逻辑的建议后，粘贴回 `NEXT_STEP.md`，让 Cursor 执行修复。
...
3.  **求助**：你复制这封"邮件"，发送给外部干净环境：**Gemini 3 Pro**、**Claude 4.5**、**DeepSeek V3.2**、**Qwen Max**。这些外部模型没有 Cursor 的历史包袱，也没有复杂的 Agent 工具干扰，它们看到的只有你提供的干净事实。
